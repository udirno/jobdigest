...
User message:
- Per user decision: send core sections only, skip benefits/perks/culture fluff to reduce token usage.
- Create helper function `extractJobCore(job)` that programmatically extracts core sections and strips fluff:
  1. Always include: `job.title`, `job.company`
  2. Extract core content from `job.description` by:
     - Splitting the description into sections (look for headings like "Requirements", "Qualifications", "Responsibilities", "Skills", "What you'll do", "About the role", "Experience", etc.)
     - KEEPING sections related to: requirements, qualifications, responsibilities, skills, experience, "what you'll do", "about the role", key duties
     - STRIPPING sections related to: benefits, perks, "why join us", "about us", company culture, diversity statements, equal opportunity, compensation/salary details (unless it's just the range), "what we offer"
     - If no clear section headings are found (unstructured description), truncate to the first 2000 characters as a reasonable approximation (requirements typically appear first in job postings)
  3. Include `job.location` and `job.salary_min`/`job.salary_max` if available (compact metadata, not fluff)
  - Format the user message as: "Job Title: {title}\nCompany: {company}\nLocation: {location}\nSalary: {range}\n\nJob Description:\n{extracted core sections}"
...
8. `extractJobCore` strips benefits/perks/culture sections per user decision (does NOT send full description text)
9. `extractJobCore` keeps requirements, qualifications, responsibilities, skills sections
10. Unstructured descriptions are truncated (not sent in full)
  </verify>
  <done>
claude-client.js exports scoreJob function that calls Claude API with structured outputs and prompt caching, handles errors gracefully, and returns scoring object with score (0-100), reasoning, and 5 dimension scores. The extractJobCore helper sends only core job sections (title, requirements, responsibilities, skills) per user decision -- benefits, perks, and culture fluff are stripped to reduce token usage.
  </done>
</task>
...
4. claude-client.js extractJobCore strips benefits/perks/culture sections per user decision (does NOT send full description)
5. job-scorer.js filters for unscored jobs and processes sequentially
6. job-scorer.js saves after each score for checkpoint resilience
7. Error handling is graceful -- no unhandled throws that would crash the pipeline
8. No circular dependencies between the two new modules
</verification>
...
- extractJobCore sends only core sections (title, requirements, responsibilities, skills) -- benefits/perks/culture stripped per user decision
