...
2. In `executeFetchPipeline()`, after the final `updateAdaptiveMetrics()` call and daily stats update, add a scoring stage:
   ...
   CRITICAL: Scoring failure must NOT cause the fetch pipeline to fail. The fetch result should still be 'success' if jobs were fetched, even if scoring fails. Wrap in try/catch and push error to historyEntry.errors. The try/catch must NOT re-throw the error -- it catches, logs, records the error, and execution continues normally to the return statement.
...
3. Scoring is wrapped in try/catch that catches errors
4. Caught scoring errors are pushed to historyEntry.errors (not silently swallowed)
5. Scoring try/catch does NOT re-throw errors -- no `throw` statement inside the catch block (scoring failure is isolated from fetch success)
6. scoringResult is included in return objects from runJobFetch
7. The 4-stage checkpoint switch (bootstrap-adzuna through remaining-fetch) is unchanged
8. Keep-alive already covers scoring (no new keep-alive calls added)
  </verify>
  <done>
job-fetcher.js pipeline runs scoring after fetching. Scoring failures are isolated -- they do not crash the fetch pipeline. The try/catch catches errors, records them in historyEntry.errors, and does NOT re-throw. Scoring results are included in the fetch return object for UI consumption.
  </done>
</task>
...
2. Scoring failure isolation: fetch succeeds even if scoring fails (try/catch with no re-throw)
...
- Scoring failures do not crash or fail the fetch pipeline (try/catch with no re-throw)
